...
def combine_and_sort_rules(rules):
    ...
    def sort_key(item):
        ...
        if key == (work_generic_project, work_generic_description):
            ...
            return ("zzzzzzz", key[1])  # 'z' will sort these last, ordered by description
        return key
...
def print_summary_and_maybe_process_actions(action_stack, rules):
    ...
    user_choice = input("Please choose an option: 'e' to save and exit, 'q' to quit without saving: ").strip().lower()
    while True:
        if user_choice == "e":
...
def process_action_stack_to_json_and_ensure_rules_match(action_stack, rules):
    ...
    if not rules_are_same(loaded_rules_from_json, rules):
        print(
            "WARNING: rules added do not correspond to rules in program's memory after saving. You might need to fix the json directly. Sorry about that."
        )
    ...
    with open(CLOCKIFY_CATEGORIES_LOCATION, "r+") as read_file:
        ...
        if category_name.split("*")[1] == "personal":
            ...
            new_rule = {
                ...
                "rule": {"type": "regex", "regex": re.escape(new_pattern), "ignore_case": not case_sensitive},
                "data": {
                    # no color for now
...
def add_new_rule(rules, new_category_name, new_regex_pattern, ignore_case=False):
    ...
    Parameters:
        rules (list): The current list of rules, where each element is a tuple containing the category name and a Rule object.
...
def get_input_from_user(description, details, rules):
    ...
    while True:
        ...
        response = (
            input(
                "\nEnter the rule number to categorize this event, 'u' to undo, 'n' to create new rule,'s' to skip, 'e' to save and exit, or q to quit without saving: "
            )
            .strip()
            .lower()
        )
        if response.isdigit():
            ...
            if not regex_to_add:
                regex_to_add = description.strip()
            return "new", {"category_name": name + "*" + project_id, "regex_to_add": regex_to_add}
        elif response == "e":
